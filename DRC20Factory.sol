...
contract DRC20Factory  {
    ...
    bytes32 private constant DOMAIN_NAME = keccak256("DualBit");
    ...
    event DRC20Created(address indexed sender, address indexed dog20);
    ...
    function createDRC20(string memory name, string memory symbol, uint8 decimals) external onlyOwner returns (address drc20) {
        parameters = Parameters({name: name, symbol: symbol, decimals: decimals});
        drc20 = address(new DRC20{salt: keccak256(abi.encode(name, symbol, decimals))}());
        delete parameters;
        emit DRC20Created(msg.sender, drc20);
    }
    ...
    function burn(address token, uint256 amount, string memory receiver) external payable nonReentrant {
        ...
        DRC20(token).transferFrom(msg.sender, address(this), amount);
        DRC20(token).burn(amount);
